...
class RxSwift8ViewController: BaseViewController {
...
//        scan()
    ...
    func concatMap() {
        ...
        subject2.onNext("2")
        subject1.onNext("C")
        subject1.onCompleted()  // 只有前一个序列结束后，才能开始接收下一个序列
    }
    ...
    func groupBy() {
        Observable<Int>.of(0, 1, 2, 3, 4, 5)
            .groupBy(keySelector: {(elem) -> String in
                return elem % 2 == 0 ? "奇数" : "偶数"
            })
            .subscribe{ (event) in
                switch event {
                case .next(let group):
